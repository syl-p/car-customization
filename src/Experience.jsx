...
const Rig = () => {
  ...
  useEffect(() => {
    switch (location) {
      case "/exterior":
        bounds.to({ position: [6, 0, 10], target: colorPosition })
        break;
      case "/exterior/materials":
        bounds.to({ position: [0, 5, 14], target: topPosition })
        break;
      case "/exterior/wheels":
        bounds.to({ position: [1, 0, 6], target: tirePosition })
        break;
      case "/interior":
...
export default function Experience() {
  ...
  return <>
    <Bounds fit clip observe damping={20} margin={0.8}>
      <McLaren scale={1.5} position={[0, -0.18, 0]} rotation={[0, -Math.PI / 5, 0]} />
      <Rig />
      <Hud />
    </Bounds>
    <hemisphereLight intensity={0.5} />
    <ContactShadows position={[0, -0.20, 0]} renderOrder={2} frames={1} resolution={1024} scale={12} blur={2} opacity={0.8} far={100} />
